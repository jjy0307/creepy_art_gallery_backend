...
def upload(request):
    ...
    images_slice = ImageModel.objects.all().order_by(
        '-timestamp')[0:2]  # 가장 최근에 넣은 input 이미지 url
    # print(images_slice)
    input_images_list = []
    for image_num in (0, 2):
        try:
            images_values = ImageModel.objects.values(
                'images').order_by('-timestamp')[image_num]['images']
            input_images_list.append(images_values)
        except:
            pass
    print("input_images_list", input_images_list)
    try:
        result_image_path = change_image(
            input_images_list[0], input_images_list[1])
        result_image_path
